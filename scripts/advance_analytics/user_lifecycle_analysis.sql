...
LEFT JOIN saas_product.dim_events e ON s.user_id = e.user_id;
...
WITH users_cte AS (
    ...
    SELECT
        signup_month,
        COUNT(DISTINCT user_id) AS signups_number
...
FROM saas_product.dim_subscriptions;
WHERE signup_date is NOT NULL OR churn_date IS NOT NULL;
...
cohort_activity AS (
    SELECT
        uc.cohort_month,
        a.activity_month,
        TIMESTAMPDIFF(MONTH, STR_TO_DATE(uc.cohort_month, '-01'), '%Y-%m-%d'),
