...
DROP VIEW IF EXISTS saas_product.plan_summary_report_saas;

CREATE VIEW saas_product.plan_summary_report_saas AS
WITH base AS (
    SELECT
        plan,
        ...
        COUNT(DISTICT user_id) - COUNT(DISTINCT CASE 
            WHEN churn_date IS NULL OR churn_date = 'n/a' THEN user_id)
