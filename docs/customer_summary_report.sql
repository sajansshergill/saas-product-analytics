...
DROP VIEW IF EXISTS saas_product.summary_report;

CREATE VIEW saas_product.summary_report AS
WITH base AS (
    SELECT
        s.user_id,
        s.plan,
...
SELECT
    ...
    e.first_event,
    e.last_event,
    FLOOR(DATEDIFF(CURRENT_DATE, e.last_event)) AS months_since_last_activity,
...
FROM base b
LEFT JOIN events.agg e ON b.user_id = e.user_id;
